                    ...
                    <tbody>
                        @foreach ($hd as $item)
                            <tr>
                                ...
                                <td>
                                    @if($item->pur_purchaseorder_status_id == 1 || $item->pur_purchaseorder_status_id == 10|| $item->pur_purchaseorder_status_id == 2)
                                ...
                                <td>{{$item->vd_vendor_code}}/{{$item->vd_vendor_fullname}}</td>
                                <td>{{$item->pur_purchaseorder_hd_save}}</td>
                                <td>{{$item->pur_purchaseorder_hd_note}}</td>
                                {{-- <td>{{number_format($item->pur_purchaseorder_hd_basevat,2)}}</td>
                                <td>{{number_format($item->pur_purchaseorder_hd_vat,2)}}</td> --}}
                                <td>{{number_format($item->pur_purchaseorder_hd_netamount,2)}}</td>
                                <td>{{number_format($item->pur_purchaseorder_hd_discount,2)}}</td>
                                {{-- <td>{{$item->approved1_save}}/{{$item->approved1_remark}}</td> --}}
                                <td>{{$item->approved2_save}}/{{$item->approved2_remark}}</td>
                                <td>
                                    @if ($item->pur_purchaseorder_status_id == 6)
                                    <a href="{{route('pur-po.edit',$item->pur_purchaseorder_hd_id)}}" class="btn btn-warning btn-sm"><i class="fas fa-eye"></i></a>
                                    @elseif($item->pur_purchaseorder_status_id == 7)
                                    @if ($item->check_approved4 == true)
                                    @if ($item->approved4_code == null)
                                    <a href="{{route('pur-po.edit',$item->pur_purchaseorder_hd_id)}}" class="btn btn-warning btn-sm"><i class="fas fa-eye"></i></a>
                                    @else
                                    <a href="javascript:void(0)" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target=".bs-example-modal-center" onclick="getDataPo('{{ $item->pur_purchaseorder_hd_id }}')"><i class="fas fa-eye"></i></a>
                                    @endif                         
                                ...
                                <td>
                                    @if ($item->pur_purchaseorder_status_id == 7)
                                    @if ($item->approved3_code == Auth::user()->emp_person_code )
                                    <a href="javascript:void(0)" class="btn btn-danger btn-sm" onclick="confirmDelPo('{{ $item->pur_purchaseorder_hd_id }}')"><i class="fas fa-trash"></i></a> 
                                    @endif
                                @endif
                                </td>
                            </tr>
...
$.ajax({
    ...
    success: function(data) {
        ...
        let el_list = '';
        let unique = [...new Map(data.dt.map(item => [item['pur_purchaseorder_dt_listno'], item])).values()];
        $.each(unique, function(key, item) {
            el_list += `
                    <tr>
                                    <th scope="row">${ item.pur_purchaseorder_dt_listno }</th>
                                    <td>${ item.pur_purchaserequest_hd_docuno }</td>
                                    <td>${ item.productcode }</td>
                                    <td>${ item.productname }</td>
                                    <td>${ parseFloat(item.pur_purchaseorder_dt_qty).toFixed(2) }</td>
                                    <td>${ item.productunit }</td>
                                    <td>${ parseFloat(item.pd_product_min).toFixed(2) }</td>
                                    <td>${ parseFloat(item.pd_product_max).toFixed(2) }</td>
                                    <td>${ parseFloat(item.pur_purchaseorder_dt_price).toFixed(2) }</td>
                                    <td>${ parseFloat(item.pur_purchaseorder_dt_netamount).toFixed(2) }</td>
                                    <td>${ parseFloat(item.pur_purchaseorder_dt_discount).toFixed(2) }</td>
                                </tr>`
        })
